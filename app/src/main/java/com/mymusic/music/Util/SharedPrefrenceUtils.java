...
public class SharedPrefrenceUtils {
    ...
    /**
     * 清除数据
     * 用Apply()先操作内存，再操作磁盘，性能高，效率低
     * @param context
     * @param key
     */
    public static void clearn(Context context,String key){
        if(sp == null){
            sp = context.getSharedPreferences("config", Context.MODE_PRIVATE);
        }
    ...
    public static void putIntegerList(Context context, String key, List<Integer> list) {
        try {
    ...
    public static List<Integer> getIntegerList(Context context, String key) {
        try {
            return (List<Integer>) get(context, key);
        } catch (Exception e) {
            e.printStackTrace();
        }
        return null;
    ...
    //图片
    private static void putimage(Context context, String key , Bitmap bitmap){
        ByteArrayOutputStream baos2 = new ByteArrayOutputStream();
        bitmap.compress(Bitmap.CompressFormat.JPEG, 50, baos2);
        String imageBase64 =new String(Base64.encode(baos2.toByteArray(), Base64.DEFAULT));
        saveString(context,key,imageBase64);
    }

    private static Bitmap getimage(Context context, String key){
        String string = getString(context, key);
        if (TextUtils.isEmpty(string)) { //不可少，否则在下面会报java.io.StreamCorruptedException
            return null;
        }
